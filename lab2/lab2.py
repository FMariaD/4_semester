...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.velocity = 40*np.ones(shape=(3, int(len(nodes_coords) / 3)), dtype=np.double)
        self.velocity = self.velocity * self.nodes 
        tmp = (np.power(self.smth, -0.5)).T
        tmp = np.vstack((tmp, (np.power(self.smth, -0.5)).T))
        tmp = np.vstack((tmp, (np.power(self.smth, -0.5)).T))
        self.velocity *= tmp
        tmp = self.nodes[1, :]
        tmp = np.vstack((tmp, (-1)*self.nodes[0, :]))
        tmp = np.vstack((tmp, np.zeros(shape=(1, len(self.nodes[1, :])), dtype=np.double)))
        tmp *= 2*np.pi
        self.velocity += tmp
    ...
    def move(self, tau):
        ...
        # Тут может быть скорость, но сейчас здесь нули
        self.velocity = 40*np.ones(shape=(3, len(self.nodes[1, :])), dtype=np.double)
        self.velocity = self.velocity * self.nodes 
        tmp = (np.power(self.smth, -0.5)).T
        tmp = np.vstack((tmp, (np.power(self.smth, -0.5)).T))
        tmp = np.vstack((tmp, (np.power(self.smth, -0.5)).T))
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("duck" + str(snap_number) + ".vtu")
        writer.Write()
...
gmsh.option.setNumber("Mesh.CharacteristicLengthFromCurvature", 25)
